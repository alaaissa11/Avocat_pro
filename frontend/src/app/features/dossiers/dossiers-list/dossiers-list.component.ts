      ...
      <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4 animate-fade-in" (click)="closeModal()">
        <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden animate-scale-in" (click)="$event.stopPropagation()">
          ...
          <div class="p-4 border-t border-slate-200 bg-slate-50 flex justify-between gap-3">
            <div>
              @if (canCloturer()) {
                <button (click)="cloturerDossier()" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all flex items-center gap-2">
...
export class DossiersListComponent implements OnInit {
  ...
  canCloturer(): boolean {
    const dossier = this.selectedDossier();
    if (!dossier || dossier.statut === 'cloture') return false;
  ...
  cloturerDossier() {
    const dossier = this.selectedDossier();
    if (!dossier) return;
    if (!confirm(`Êtes-vous sûr de vouloir clôturer le dossier "${dossier.numero}" ? Cette action archivéra toutes les tâches et documents dans l'historique.`)) return;
    this.dossierService.cloturerDossier(dossier._id).subscribe({
      next: () => {
        this.closeModal();
        this.loadDossiers();
        this.loadStats();
      },
      error: (err) => console.error('Error closing dossier:', err)
    });
  }
